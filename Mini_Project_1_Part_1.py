...
def cosine_similarity(x, y):
    ...
    cosine_sim = dot_product / (norm_y * norm_x)
...
def averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type=50):
    ...
    for word in words:
        if word in word_index_dict:
            ...
            word_embeddings = embeddings[word_index]
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        for category in categories:
            if category in word_index_dict:
                category_index = word_index_dict[category]
                category_embedding = embeddings[category_index]
            else:
                category_embedding = np.zeros(int(model_type.split("d")[0]))

            category_embeddings[category] = category_embedding

        for category, category_embedding in category_embeddings.items():
            sim = cosine_similarity(input_embedding, category_embedding)
            cosine_sim[category] = sim
        ...
        for category in categories:
            if category in category_embeddings:
                ...
                cosine_sim[category] = 1
                print(f"Warning: Embedding for category '{category}' not found. Assigned similarity of 1.")
